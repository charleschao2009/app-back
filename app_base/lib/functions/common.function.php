...
function load_class($class)
{
	if (!class_exists($class)) {
		$class_file = CLASS_DIR . $class.'.class.php';
 ...
 * 例子：
 * $arr="default=淡蓝(默认)|mac=mac海洋";
 * $tpl="<li class='list {this}' theme='{0}'>{1}</li>\n";
 * echo getTplList('|','=',$arr,$tpl,'mac','this');
 * 输出为<li class='list ' theme='default'>淡蓝(默认)</li>
 * <li class='list this' theme='mac'>mac海洋</li>
 */
function getTplList($cute1, $cute2, $arraylist, $tpl, $this = "", $this_str = 'this')
{
	...
	foreach ($list as $value) {
		...
		if ($this == "") {
			$temp = str_replace(array('{0}', '{1}'), $info, $tpl);
		} else {
			if ($info[0] == $this) {
				$temp = str_replace(array('{0}', '{1}'), $info, $tpl);
 ...
 * @return string 
 */
function rand_string($len = 4, $type = 5, $addChars = '')
{
	$str = '';
	switch ($type) {
		case 0://大小写中英文
